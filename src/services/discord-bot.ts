  ...
  Effect.gen(function* () {
        ...
        [...grouped.entries()].map(async ([userId, hs]) => {
          ...
          return (
            ...
            hs
              .map(
                (h) =>
                  `  [${h.handle}](<${profileUrl(h.platform, h.handle)}>)  ${platformLabel(h.platform)}`,
              )
              .join("\n")
          );
        }),
    ...
    onCommand("/compare", async (event, post) => {
      ...
      const fmt = (p: typeof left) =>
        `> **[${p.handle}](<${p.profileUrl}>)**  \`${p.rating ?? "Unrated"}\`${p.rankLabel ? `  *${p.rankLabel}*` : ""}${p.maxRating ? `  ·  max \`${p.maxRating}\`` : ""}`;
      await post(
        [
